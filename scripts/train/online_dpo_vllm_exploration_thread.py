...
from utils import (
    ...
    SFTPromptExplorationDatasetProcessor,
    SimpleGenerateWithExplorationCollator
)
...
class Args:
    ...
    """What train step frequency to save the model at"""
...
def main(args: Args, dataset_config: DatasetConfig, model_config: ModelConfig):
    ...
    print("############################################")
    print(f"cuda:{accelerator.local_process_index}")
    print("############################################")
    ...
    dataset_processor = SFTPromptExplorationDatasetProcessor(tokenizer=tokenizer, config=dataset_config)
    ...
    if args.dataset_eval_mixer is not None:
        ...
        # TODO: Remove data file hardcode
        #eval_dataset = load_dataset('parquet', data_files=f"data/alfworld/prm/20241230-iter0-iter1/test.parquet", split='train')
    ...
    data_collator = SimpleGenerateWithExplorationCollator(pad_token_id=tokenizer.pad_token_id)
    ...
    if accelerator.is_main_process:
        ...
        if eval_dataset is not None:
            sample_evaluation_prompt_nominal_token_ids = eval_dataset[:num_eval_samples][INPUT_IDS_PROMPT_KEY]
            sample_evaluation_prompt_nominal_token_ids = [x for x in sample_evaluation_prompt_nominal_token_ids for _ in range(args.num_generation_per_prompt)]
            sample_evaluation_prompt_exploration_token_ids = eval_dataset[:num_eval_samples][INPUT_IDS_PROMPT_EXPLORATION_KEY]
            sample_evaluation_prompt_exploration_token_ids = [x for x in sample_evaluation_prompt_exploration_token_ids for _ in range(args.num_generation_per_prompt)]
            sample_evaluation_prompt_token_ids = [item for pair in zip(sample_evaluation_prompt_nominal_token_ids, sample_evaluation_prompt_exploration_token_ids) for item in pair]
    ...
    if random_value < args.exploration_prob:
        send_queries(accelerator, None, tokenizer, param_prompt_Q, queries_next_exploration)
    else:
    ...
    for training_step in range(resume_training_step, args.num_training_steps + 1):
        ...
        with unwrap_model_for_generation(model, accelerator) as unwrapped_model: 
            ...
            with torch.no_grad():
                ...
                if accelerator.is_main_process:
                    ...
                    #         item = item[:args.response_length]
                    ...
                    for i, item in enumerate(g_padded_response_ids):
                        if len(item) > args.response_length:
                            g_padded_response_ids[i] = item[:args.response_length]
                        assert len(g_padded_response_ids[i]) == args.response_length
                        for inner_item in g_padded_response_ids[i]:
                            if not inner_item < config.vocab_size:
                                assert inner_item < config.vocab_size, f"{inner_item=}, {tokenizer.vocab_size=}"
                    g_padded_response_ids = torch.tensor(g_padded_response_ids, device=device)
        ...
        with torch.no_grad():
            ...
            local_metrics[17] = torch.sigmoid((chosen_rewards_stats - rejected_rewards_stats) / 1.0).mean()
            ...
            metrics = {
                ...
                "rewards/soft_accuracies": global_metrics[17],
