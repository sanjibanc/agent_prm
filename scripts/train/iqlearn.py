...
def evaluate(
    model: PreTrainedModel, dataloader: DataLoader, tokenizer: PreTrainedTokenizer, max_sampled_texts: int = 0, gamma = 1.0
) -> Tuple[dict, dict]:
    model.eval()
    total_loss = 0
    total_accuracy = 0
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            current_queries = data[INPUT_IDS_PROMPT_CURRENT_KEY]
            future_queries = data[INPUT_IDS_PROMPT_FUTURE_KEY] 
            labels = data[LABEL_KEY]
            prompts = data[PROMPT_CURRENT_KEY]
            completions = data[COMPLETION_CURRENT_KEY]
            
            _, current_q_val, _ = get_reward(model, current_queries, tokenizer.pad_token_id, 0)
            _, future_q_val, _ = get_reward(model, future_queries, tokenizer.pad_token_id, 0)
            predicted_reward_non_terminal = current_q_val - gamma*future_q_val
            predicted_reward_terminal = current_q_val
            predicted_reward = (~data[TERMINAL_KEY])*predicted_reward_non_terminal + data[TERMINAL_KEY]*predicted_reward_terminal
            ...
            if table is not None:
                ...
                table["prompt_completion"].extend(tokenizer.batch_decode(current_queries))
                table["label"].extend(labels.tolist())
                table["q_val"].extend(current_q_val.tolist())
...
def main(args: Args, dataset_config: DatasetConfig, model_config: ModelConfig):
    ...
    dataset_processor = IQLearnDatasetProcessor(tokenizer=tokenizer, config=dataset_config)
    ...
    data_collator = IQLearnCollator(pad_token_id=tokenizer.pad_token_id)
    ...
    for epoch_id in range(args.num_train_epochs):
        for data in dataloader:
            ...
            with accelerator.accumulate(model):
                ...
                _, current_q_val, _ = get_reward(model, current_queries, tokenizer.pad_token_id, 0)
                _, future_q_val, _ = get_reward(model, future_queries, tokenizer.pad_token_id, 0)
                predicted_reward_non_terminal = current_q_val - args.gamma*future_q_val
                predicted_reward_terminal = current_q_val
                predicted_reward = (~data[TERMINAL_KEY])*predicted_reward_non_terminal + data[TERMINAL_KEY]*predicted_reward_terminal
            ...
            if (training_step==1) or (args.num_evals > 0 and training_step % args.eval_freq == 0):
                eval_metrics, table = evaluate(model, eval_dataloader, tokenizer, max_sampled_texts=10, gamma=args.gamma)
                for key in table:
